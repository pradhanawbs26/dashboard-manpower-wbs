...
import { SpreadsheetBreakdown } from "../utils/googleSheets";
...
interface FieldMonitorProps {
  ...
  spreadsheetBreakdowns?: SpreadsheetBreakdown[];
  isSyncingSheets?: boolean;
  onSyncSheets?: () => void;
  sheetsConnected?: boolean;
  sheetsError?: string | null;
  lastSyncedSheets?: string | null;
}
...
export default function FieldMonitor({
  ...
  setSelectedDate,
  onNavigateToSetting,
  spreadsheetBreakdowns = [],
  isSyncingSheets = false,
  onSyncSheets,
  sheetsConnected = false,
  sheetsError = null,
  lastSyncedSheets = null,
}: FieldMonitorProps) {
  ...
  // Find active spreadsheet breakdown for a given unit code matching target date & shift
  const getActiveSpreadsheetBreakdown = (unitCode: string) => {
    if (
      !spreadsheetBreakdowns ||
      spreadsheetBreakdowns.length === 0 ||
      !unitCode
    )
      return null;
    return (
      spreadsheetBreakdowns.find((b) => {
        if (!b.appUnitCode) return false;
        const cleanA = b.appUnitCode.replace(/\s+/g, "").toUpperCase();
        const cleanB = unitCode.replace(/\s+/g, "").toUpperCase();
        if (cleanA !== cleanB) return false;

        const dateMatch = b.tanggal === selectedDate;
        if (!dateMatch) return false;

        const shiftMatch = b.shift === selectedShift;
        if (!shiftMatch) return false;

        return b.status === "BREAKDOWN";
      }) || null
    );
  };
  ...
  const resolvedSettings = useMemo(() => {
    ...
    const activeTransfersForThisShift = (backupTransfers || []).filter(
      (bt) =>
        bt.date === selectedDate && Number(bt.shift) === Number(selectedShift),
    );
    ...
    const initialUtama = settings
      ...
      .map((setting) => {
        ...
        const isConfiguredEmpty =
          selectedShift === 1 ? !setting.operator1Id : !setting.operator2Id;
        ...
        if (
          activeOperator &&
          operatorTransferTargetMap.has(activeOperator.id)
        ) {
        ...
        const transferredInOps = unitTransferredOperatorsListMap.get(
          setting.unitId,
        );
        ...
        const isPrimaryActive =
          ...
          activeRoleStatus !== "OFF";
        const sheetBd = unit
          ? getActiveSpreadsheetBreakdown(unit.unitCode)
          : null;
        const isUnitBroken =
          (unit &&
            (unit.status === "Breakdown" || unit.status === "Maintenance")) ||
          !!sheetBd;
    ...
    const rawUnconfiguredUnits = units.filter(
      (u) => !configuredUnitIds.has(u.id),
    );
    ...
    const targetUnconfiguredItems = rawUnconfiguredUnits.map((unit) => {
      ...
      const sheetBd = getActiveSpreadsheetBreakdown(unit.unitCode);
      const isUnitBroken =
    ...
    const allTargets = [...initialUtama, ...targetUnconfiguredItems];
    ...
    initialUtama.forEach((item) => {
      if (
        item.isPrimaryActive &&
        item.isUnitBroken &&
        !item.isFilledByBackupTransfer &&
        !item.isTransferredOut
      ) {
    ...
    settings.forEach((s) => {
      if (s.groupId === "master") {
        ...
        if (
          activeMop &&
          activeMop.status === "Active" &&
          activeRole !== "OFF"
        ) {
          const isManuallyTransferred = operatorTransferTargetMap.has(
            activeMop.id,
          );
    ...
    allTargets.forEach((item) => {
      ...
      if (isVacantAndReady && item.unit) {
        const scoredCandidates = availableMasterCandidates
          .filter((cand) => !cand.isAssignedToUnitCode)
          .filter((cand) => {
            const hasSpecialization = (cand.specializations || []).includes(
              item.unit!.type,
            );
            const hasPriorityUnit1 =
              cand.backupPriorityUnitId1 === item.unit!.id;
            const hasPriorityUnit2 =
              cand.backupPriorityUnitId2 === item.unit!.id;
            const hasPriorityType1 =
              cand.backupPriorityType1 === item.unit!.type;
            const hasPriorityType2 =
              cand.backupPriorityType2 === item.unit!.type;
            return (
              hasSpecialization ||
              hasPriorityUnit1 ||
              hasPriorityUnit2 ||
    ...
    const mappedMasterSettings = settings
      ...
      .map((setting) => {
        ...
        const dispatchedTo =
          autoDispatched ||
          (isTransferredOut ? manualTransferUnitCode : undefined);

        const opSpecializationsStr =
          op1?.specializations?.join(", ") || "Dump Truck";
        ...
        return {
          ...
          activeOperator:
            activeOperator ||
            (isTransferredOut
              ? employeeMap.get(setting.operator1Id) ||
                employeeMap.get(setting.operator2Id) ||
                null
              : null),
          activeRoleStatus: isOff
            ? ("OFF" as const)
            : selectedShift === 1
              ? ("S" as const)
              : ("M" as const),
    ...
    const allUtamaFilledByPrimary =
      mappedUtamaSettings.length > 0 &&
      mappedUtamaSettings.every((item) => {
        if (item.isUnitBroken) return true; // ignore broken units from this block

        return (
          item.originalOperator &&
  ...
  const filteredResolvedSettings = useMemo(() => {
    return resolvedSettings.filter((item) => {
      ...
      const matchesSearch =
        ...
        (item.activeOperator?.name || "TIDAK ADA OPERATOR")
          .toLowerCase()
      ...
      const matchesGroup =
        ...
        item.setting.groupId === selectedGroupFilter;
  ...
  const unconfiguredUnits = useMemo(() => {
    ...
    const filledUnconfiguredUnitIds = new Set(
      resolvedSettings
        .filter(
          (item) => (item as any).isUnconfiguredTarget && item.activeOperator,
        )
    ...
    return units.filter((u) => {
      ...
      return (
        ...
        !filledUnconfiguredUnitIds.has(u.id) &&
        matchesSearch
      );
    });
    ...
    settings.forEach((s) => {
      if (s.groupId === "master") {
        ...
        if (
          ...
          const activeTransfersForThisShift = (backupTransfers || []).filter(
            (bt) =>
              bt.date === selectedDate &&
              Number(bt.shift) === Number(selectedShift),
          );
          const isManuallyTransferred = activeTransfersForThisShift.some(
            (bt) => bt.operatorId === activeMop!.id,
          );

          if (!isManuallyTransferred) {
            // Check if automatically assigned to backup any vacant unit
            const slotCode = s.masterSlotCode || "M-1";
            const isAutoDispatched = resolvedSettings.some(
              (item) =>
            ...
            if (!isAutoDispatched) {
              const priorities = [
                s.backupPriorityType1,
                s.backupPriorityType2,
  ...
  const filteredStandbyMasters = useMemo(() => {
    return standbyMastersList.filter((m) => {
      ...
      const matchesGroup =
        selectedGroupFilter === "all" ||
        selectedGroupFilter === "master" ||
        selectedGroupFilter === "utama";
      return matchesSearch && matchesGroup;
    });
  ...
  return (
    <div
      className="flex flex-col h-full bg-slate-50 text-slate-700"
      id="field-monitor-container"
    >
      {/* 1. Header Board */}
      <div className="bg-white text-slate-800 p-5 border-b border-slate-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 md:mx-auto border-t md:border-t-0 pt-3 md:pt-0 border-slate-150">
            <div className="flex flex-col text-left mr-1 justify-center">
              <div className="flex items-center gap-1.5">
                <FileSpreadsheet
                  className={`h-4 w-4 shrink-0 ${sheetsConnected ? "text-emerald-600" : "text-slate-400"}`}
                />
                <span className="text-xs font-black text-slate-800 uppercase tracking-wide">
                  SINKRONISASI SHEETS
                </span>
                <span
                  className={`h-1.5 w-1.5 rounded-full ${sheetsConnected ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
                />
              </div>
              <p className="text-[10px] text-slate-500 font-mono">
                {lastSyncedSheets
                  ? `Sinkron: ${new Date(lastSyncedSheets).toLocaleTimeString("id-ID")} (${new Date(lastSyncedSheets).toLocaleDateString("id-ID")})`
                  : "Belum terhubung"}
              </p>
            </div>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold font-mono transition-all border shadow-sm select-none cursor-pointer ${
                isSyncingSheets
                  ? "bg-slate-100 border-slate-250 text-slate-455 cursor-not-allowed"
                  : "bg-emerald-50 border-emerald-300 text-emerald-800 hover:bg-emerald-100/70 hover:border-emerald-500 active:scale-95"
              }`}
              title="Hubungkan Google Sheets & Unduh Database Kerusakan"
            >
              <RefreshCw
                className={`h-3.5 w-3.5 shrink-0 ${isSyncingSheets ? "animate-spin" : ""}`}
              />
              <span>
                {isSyncingSheets ? "Menyinkronkan..." : "Sinkron Sheets"}
              </span>
            </button>
          </div>
        ...
        {sheetsError && (
          <div className="mt-3 flex items-start gap-2.5 p-3 rounded-lg bg-rose-50 border border-rose-200 text-rose-900 text-xs shadow-sm">
            <AlertCircle className="h-4 w-4 shrink-0 text-rose-500 mt-0.5 animate-bounce" />
            <div className="flex flex-col gap-0.5 w-full">
              <span className="font-black font-mono tracking-tight text-[11px] uppercase text-rose-850">
                Gagal sinkronisasi google sheets:
              </span>
              <p className="font-medium font-sans text-[11.5px] leading-relaxed text-rose-800">
                {sheetsError}
              </p>
            </div>
          </div>
      ...
      <div
        className="bg-white p-4 border-b border-slate-200 flex flex-col md:flex-row gap-3 shadow-sm"
        id="board-controls-area"
      >
      ...
      <div
        className="flex-1 p-6 overflow-y-auto bg-slate-100"
        id="field-monitor-scroll-area"
      >
        {/* If no configurations match and no unconfigured units */}
        {filteredResolvedSettings.length === 0 &&
          unconfiguredUnits.length === 0 && (
            <div className="text-center py-12 bg-white rounded-lg border border-dashed border-slate-300 p-8 max-w-md mx-auto mt-6">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Coba sesuaikan pencarian atau filter grup Anda.
              </p>
            </div>
          )}
        ...
        {categorizedUnits.map((category) => {
          return (
            <div
              ...
              id={`category-block-${category.type.toLowerCase().replace(/\s+/g, "-")}`}
            >
              ...
              <div className="flex items-center gap-2 mb-3 select-none">
                ...
                <span className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">
                  {category.resolvedSettings.length +
                    category.unconfigured.length}{" "}
                  Unit
                </span>
              </div>
                  ...
                  ({
                    ...
                    return (
                          ...
                          isUnitBroken
                            ...
                            : isOff
                              ...
                              : isMasterGroup
                                ...
                                : isFilledByBreakdownRelocation
                                  ? selectedShift === 1
                                    ? "border-amber-400 hover:border-amber-550 hover:shadow-md bg-amber-50/15"
                                    : "border-indigo-400 hover:border-indigo-550 hover:shadow-md bg-indigo-50/15"
                                  : selectedShift === 2
                                    ...
                                    : isFilledByMaster
                                      ...
                                      : isFilledByBackupTransfer
                                        ? "border-indigo-300 hover:border-indigo-550 hover:shadow-md bg-indigo-50/10"
                      ...
                      >
                        ...
                        <div
                          className={`py-1.5 px-2 text-center border-b font-extrabold font-mono tracking-wider text-xs ${
                            isUnitBroken
                                ...
                                : isMasterGroup
                                  ...
                                  : isFilledByBreakdownRelocation
                                    ? selectedShift === 1
                                      ...
                                      : "bg-indigo-800 border-indigo-800 text-white font-black"
                                    : isFilledByBackupTransfer
                                      ? selectedShift === 1
                                        ? "bg-amber-550 border-amber-550 text-slate-950 font-black"
                        ...
                        <div className="flex-1 p-2 flex flex-col justify-center items-center text-center">
                          {isUnitBroken ? (
                            <div className="py-1 w-full flex flex-col justify-center items-center">
                              <div className="inline-flex items-center gap-1 py-0.5 px-1.5 rounded-full text-[8px] font-extrabold uppercase bg-rose-100 text-rose-700 border border-rose-250 mb-1 font-mono tracking-wide">
                                🚨 {sheetBd ? `${sheetBd.tipe || "BREAKDOWN"}` : unit.status.toUpperCase()}
                              </div>

                              {sheetBd ? (
                                <div className="w-full text-center">
                                  <p
                                    className="font-extrabold text-rose-700 uppercase text-[10px] tracking-tight leading-tight max-w-full truncate px-1"
                                    title={sheetBd.problem}
                                  >
                                    {sheetBd.problem || "ADA PROBLEM"}
                                  </p>
                                  <p className="text-[9px] text-slate-500 font-bold mt-0.5 truncate max-w-full px-1">
                                    📍 {sheetBd.lokasi || "Workshop"}
                                  </p>
                                  {sheetBd.deskripsi && (
                                    <p
                                      className="text-[8px] text-slate-400 font-mono italic mt-0.5 max-w-full truncate px-1 leading-tight"
                                      title={sheetBd.deskripsi}
                                    >
                                      "{sheetBd.deskripsi}"
                                    </p>
                                  )}
                                  <div className="pt-1.5 flex justify-center">
                                    <span className="inline-flex items-center gap-0.5 text-[7px] bg-emerald-50 text-emerald-700 border border-emerald-250 px-1 py-0.5 rounded-sm uppercase font-black tracking-wider font-mono">
                                      <FileSpreadsheet className="h-2.5 w-2.5 shrink-0 text-emerald-600" />
                                      SHEETS SYNC
                                    </span>
                                  </div>
                                </div>
                            ...
                            <div className="w-full py-1">
                              ...
                              <h3
                                className={`text-xs font-black tracking-tight leading-normal line-clamp-1 truncate ${
                                  isMasterGroup
                                    ? dispatchedTo
                                    ...
                                    : isFilledByBreakdownRelocation
                                      ...
                                      : isFilledByBackupTransfer
                                        ? selectedShift === 1
                                          ? "text-amber-850 font-black"
                                          : "text-indigo-850 font-black"
                                        : isFilledByMaster
                                          ? selectedShift === 1
                              ...
                              {isFilledByBreakdownRelocation && (
                                <div
                                  className={`mt-1.5 px-2 py-0.5 rounded text-[9px] font-black uppercase text-center flex items-center justify-center gap-1 shrink-0 border ${
                                    selectedShift === 1
                              ...
                              {isFilledByBackupTransfer && !isMasterGroup && (
                                <div
                                  className={`mt-1.5 px-2 py-0.5 rounded text-[10px] font-black uppercase text-center flex items-center justify-center gap-1 shrink-0 border ${
                                    selectedShift === 1
                        ...
                        <div
                          className={`px-2 py-1 border-t text-[9px] flex items-center justify-between ${
                            isOff
                        ...
                        >
                          <span className="truncate max-w-[80px] font-medium font-mono">
                            {isMasterGroup
                              ? `EGI: ${activeOperator?.specializations?.[0] || "Umum"}`
                              : unit.brand}
                          </span>
                          {isOff ? (
                            <span className="shrink-0 text-slate-400 font-bold uppercase text-[8px]">
                              OFF
                            </span>
                          ) : (
                            <span
                              className={`shrink-0 font-extrabold text-[8px] px-1 py-0.5 rounded uppercase ${
                                isMasterGroup
                                  ? dispatchedTo
                                  ...
                                  : isFilledByMaster
                                    ...
                                    : selectedShift === 1
                                      ...
                                      : "bg-indigo-100 text-indigo-805 border border-indigo-205"
                              }`}
                            >
                ...
                {category.unconfigured.map((unit) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`select-none bg-white border rounded-lg shadow-sm flex flex-col overflow-hidden transition-all duration-200 hover:shadow-md ${
                        isUnitBroken
                          ? "border-rose-350 bg-rose-50/10 hover:border-rose-400"
                    ...
                    >
                      ...
                      <div className="flex-1 p-2 flex flex-col justify-center items-center text-center">
                        {isUnitBroken ? (
                          ...
                          <div className="py-1">
                            <div className="inline-flex items-center gap-0.5 py-0.5 px-1.5 rounded-full text-[8.5px] font-bold uppercase bg-amber-50 text-amber-700 border border-amber-200/50 mb-1 font-mono">
                      ...
                      <div className="px-2 py-1 border-t text-[9px] flex items-center justify-between bg-slate-50 border-slate-200 text-slate-400">
                        ...
                        <span
                          className={`shrink-0 font-extrabold text-[8px] px-1 py-0.5 rounded uppercase border ${
                            isUnitBroken
                              ? "bg-rose-50 border-rose-200 text-rose-700"
                              : "bg-rose-50 border-rose-200 text-rose-700"
                          }`}
                        >
                          {isUnitBroken ? "RUSAK" : "KOSONG"}
                        </span>
                      </div>
                    </motion.div>
                  );
                ...
                {filteredStandbyMasters
                  .filter((m) => {
                    ...
                    if (
                      ...
                      (m.activeOperator.specializations || []).includes(
                        category.type,
                      )
                    )
                      return true;
                    if (
                      m.priorities.length === 0 &&
                      (!m.activeOperator.specializations ||
                        m.activeOperator.specializations.length === 0) &&
                      category.type === "Other"
                    ) {
                      return true;
                    }
